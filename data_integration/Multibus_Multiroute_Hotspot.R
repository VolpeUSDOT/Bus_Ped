...
use_lon_range = extendrange(d@coords[,1], f = 0.1)
use_lat_range = extendrange(d@coords[,2], f = 0.1)
...
for(warningtype in warning_keywords){ # Start loop over warning types. warningtype = 'PCW'
  ...
  usewarning = unique(d$warning_name)[grep(warningtype, unique(d$warning_name))]
  
  if(warningtype == 'All') { usewarning = unique(d$warning_name)}
