...
# Based on Mobileye Shield+ telematics data from LADOT bus 15301 January-August 2018
...
load("Event_Dist_Nearest_byHour_DASH.RData")
...
d$time <- strptime(d$LocationTime, "%Y-%m-%d %H:%M:%S")
# summary(d$time)

# Trim trailing white space from all text fields
textfields <- c("VehicleName", "Address", "StatusName")
...
d$route_id <- make.names(paste(paste(d$maj.bus.system, d$maj.nearest.route)))
