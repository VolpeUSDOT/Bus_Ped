...
dbtime <- strptime(db_d$LocationTime, "%Y-%m-%d %H:%M:%s")
db_d$hour <- format(dbtime, "%H")
...
# Checking output
kable(round(dt_dash_dist_mat[,91:95], 2), caption="Distance (in miles) between five selected events and Downtown DASH routes") %>% kable_styling(bootstrap_options = c("striped", "hover"))

# Are any distances to community DASH routes less than 1 mile?
kable(round(co_dash_dist_mat[,91:95], 2), caption="Distance (in miles) between five selected events and Community DASH routes.") %>% kable_styling(bootstrap_options = c("striped", "hover"))
...
# Checking output
kable(db_d %>% select(ll.lat, ll.lon, dt_dash_route, dt_dash_dist, co_dash_route, co_dash_dist, metro_route, metro_dist, nearest.route, bus.system) %>% sample_n(size = 5), caption="Distance (in miles) between five selected events and Downtown DASH routes") %>% kable_styling(bootstrap_options = c("striped", "hover"))
