...
if(length(grep("Basemaps", dir())) == 0){
  ...
  save(list=c("map_terrain_15", "map_toner_hybrid_15","map_toner_hybrid_14", "map_toner_13", "map_toner_12"), 
       file = "Basemaps.RData")
  } else { 
    load("Basemaps.RData") }
...
ggmap(map_toner_12, extent  = "device") +
  ...
  geom_path(data = dt_dash_merged, aes(x = long, y = lat, color = RouteID), size = 1.2) +
...
# Process to nearest route for each individual event
  ...
  xx <- min.dist.route.id 
              

route_id = vector()
  for(i in min.dist.route.id){
