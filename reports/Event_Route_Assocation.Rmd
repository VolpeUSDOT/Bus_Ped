...
for(i in 1:nrow(min.dist.route.id)){
     if(db_d[i,c("dt_dash_dist","co_dash_dist","metro_dist")][min.dist.route.num[i]] > min.dist.use){
...
# Checking output
kable(db_2 %>% 
        select(ll.lat, ll.lon, dt_dash_route, dt_dash_dist, co_dash_route, co_dash_dist, metro_route, metro_dist, nearest.route, bus.system, maj.nearest.route, maj.bus.system, confidence) %>% 
        sample_n(size = 5), caption="Distance (in miles) between five selected events and Downtown DASH routes") %>% kable_styling(bootstrap_options = c("striped", "hover"))
