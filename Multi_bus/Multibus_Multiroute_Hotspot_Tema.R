...
for(j in 5:length(month_name_list)){
  print(paste0("this is month: ", month_name_list[j]))

  load(file.path(version, paste0("Event_Dist_Nearest_byHour_DASH_", month_name_list[j], ".RData"))) # loads data.frame db_2, which includes the  nearest.route variable from the proximity analysis.
  ...
  for(warningtype in warning_keywords){ # Start loop over warning types. warningtype = 'PCW'
    ...
    if(sum(d_all$warning_name %in% usewarning) > 0 ) { # For January, no hard braking events occurred
      ...
      # Work in parallel
      cl = makeCluster(2) #parallel::detectCores())
      ...
      foreach(idx = unique(d$route_id), .packages = c("dplyr", "ggmap", 'MASS', 'raster', 'sp', 'reshape')) %dopar% { 
        ...
        # Check to see if at least one warning of this type, for this route, this month is present 
        if(!is.null(dim(usedat))) { 
        ...
        writeRaster(d_r_ll, filename = file.path('Raster', paste0(month_name_list[j], "Raster_Unnormalized_", warningtype, '_', idx, ".tif")),
                    format = 'GTiff',
                    overwrite = TRUE)
        # in ArcMap: open GeoTIFF, change symbology as appropriate.
        } # end check to see if nrow(usedat) > 0
      ...
      foreach(idx = unique(d$route_id), .packages = c("dplyr", "ggmap", 'MASS', 'raster', 'sp', 'reshape')) %dopar% { 
        ...
        # Write out to spatial data layer for ArcMap work
        writeRaster(d_r_ll, filename = file.path('Raster', paste0(month_name_list[j], "Raster_Normalized_", warningtype, '_',idx, ".tif")),
                    format = 'GTiff',
                    overwrite = TRUE)
        # in ArcMap: open GeoTIFF, change symbology as appropriate.
      } # end check for at least one warning of this warning type observed
    } # end loop over routes
    stopCluster(cl); rm(cl); gc()
  } # End check for warning type having data
